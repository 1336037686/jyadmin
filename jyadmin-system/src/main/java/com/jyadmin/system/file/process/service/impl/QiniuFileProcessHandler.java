 ...
 */
@Slf4j
@Service("qiniuFileProcessHandler")
public class QiniuFileProcessHandler implements FileProcessHandler {
    ...
    public FileProcess upload(FileProcessUploadDTO fileProcessUploadDTO, ModuleConfigWrapper fileConfigWrapper) {
        ...
        try {
            ...
            fileUrl = domain.charAt(domain.length() - 1) == '/' ? domain + putRet.key : String.join("/", domain, putRet.key);
